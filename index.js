...
addBoxButton1.addEventListener('click',() =>{
        ...
        if (parseInt(users.textContent) === 22 || parseInt(comps.textContent) === 22) {
    ...
    localStorage.setItem('usersScore', users.textContent);
        ...
        else if (parseInt(users.textContent) <= parseInt(comps.textContent)) {
            hiddenBox1.classList.add('hidden'); // Hide hiddenBox1 when users' value is equal to or less than comps' value
        }
        toggleAnimation();
        function toggleAnimation() {
            console.log("toggled");
